#' Calculate ESC-Score2 table or formula Version
#'
#' @description This function takes necessary parameters to calculate the ESC-Score2 to estimate 10-year risk of cardiovascular disease in Europe in individuals aged 40-69 years in four geographical risk regions.
#'
#' @param sex gender; categorical \[female|male\]
#' @param age age; integer \[years\]
#' @param totchol total cholesterol; numeric \[mg/dl\] or \[mmol/L\]. If unit is \[mg/dL\] set  the argument mmol to FALSE
...
#' @param risk choose which risk chart is used for calculation; categorical \["low"|"moderate"|"high"|"very high"\]
#' @param mmol is Cholesterol given as mmol/L; logical \[TRUE|FALSE\]
#' @aliases ESC_Score2_table ESC_Score2_formula
#' @usage ESC_Score2_table(sex, age, totchol, hdl, sbp, smoker, risk = c("low","moderate","high","very high"), mmol = FALSE)
#' @usage ESC_Score2_formula(sex, age, totchol, hdl, sbp, smoker, diabetic, risk = c("low","moderate","high","very high"), mmol = FALSE)
#' @return A vector of calculated 10 year risk of cardiovascular in percent.
#' @details Aims:\cr
...
#' @export
ESC_Score2_table <- function(sex, age, totchol, hdl, sbp, smoker, risk = c("low","moderate","high","very high"), mmol = FALSE) {
  ...
  D5 <- ((ifelse(female == '1', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_1 == '1', 1, 0)))
  D6 <- ((ifelse(female == '1', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_2 == '1', 1, 0)))
  D7 <- ((ifelse(female == '1', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_3 == '1', 1, 0)))
  D8 <- ((ifelse(female == '1', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_4 == '1', 1, 0)))

  D9 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '0', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_1 == '1', 1, 0)))
  D10 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '0', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_2 == '1', 1, 0)))
  D11 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '0', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_3 == '1', 1, 0)))
  D12 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '0', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_4 == '1', 1, 0)))
  D13 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_1 == '1', 1, 0)))
  D14 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_2 == '1', 1, 0)))
  D15 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_3 == '1', 1, 0)))
  D16 <- ((ifelse(female == '0', 1, 0))*(ifelse(smoker == '1', 1, 0))*(ifelse(age_6 == '1', 1, 0))*(ifelse(SBP_1 == '1', 1, 0))*(ifelse(chol_4 == '1', 1, 0)))
...
#' @export
ESC_Score2_formula <- function(sex, age, totchol, hdl, sbp, smoker, diabetic, risk = c("low","moderate","high","very high"), mmol = FALSE) {
  ...
  ESCdata <- merge(ESCdata, esc_score_2_coefficients, by = "sex", all.x = TRUE)
  ...
  sum_coefs <- (ESCdata$age-60)/5 * esc_score_2_coefficients$age_coef +
    (ESCdata$totchol-6) * esc_score_2_coefficients$totchol_coef +
    (ESCdata$hdl-1.3)/0.5 * esc_score_2_coefficients$hdl_coef +
    (ESCdata$sbp-120)/20 * esc_score_2_coefficients$sbp_coef +
    ESCdata$diabetic * esc_score_2_coefficients$diab_coef +
    ESCdata$smoker * esc_score_2_coefficients$smoker_coef +
    (ESCdata$age-60)/5 * (ESCdata$totchol-6) * esc_score_2_coefficients$totchol_age_coef +
    (ESCdata$age-60)/5 * (ESCdata$hdl-1.3)/0.5 * esc_score_2_coefficients$hdl_age_coef +
    (ESCdata$age-60)/5 * (ESCdata$sbp-120)/20 * esc_score_2_coefficients$sbp_age_coef +
    (ESCdata$age-60)/5 * ESCdata$diabetic * esc_score_2_coefficients$diab_age_coef +
    (ESCdata$age-60)/5 * ESCdata$smoker * esc_score_2_coefficients$smoker_age_coef

  ## Calcualtion of raw Risk

  ESCdata$risk_10y <- 1 - (ESCdata$baseline_surv_coef^exp(sum_coefs))
